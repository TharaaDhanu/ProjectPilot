 ...
 *  - Dashboard cards below: today's schedule, upcoming deadlines/meetings/tasks,
 *    recent activity, birthdays, employees on leave, stats
...
import {
  ...
  LuBriefcase, LuActivity, LuList,
  LuGrid3X3, LuColumns3, LuLoader, LuInbox, LuCake, LuPlane,
} from 'react-icons/lu';
import { Link } from 'react-router-dom';
import { MdHome, MdChevronRight as MdChevronRightIcon } from 'react-icons/md';
  ...
  return (
    <DashboardLayout pageTitle="Calendar">
      <div className={styles.page}>
        ...
        {/* ── Dashboard Cards (Below Calendar) ── */}
        <div className={styles.dashboardCards}>
          <DashboardCard
            icon={LuClock}
            title="Today's Schedule"
            loading={dashLoading}
            empty={!dashboard?.today_schedule?.length}
            emptyText="Nothing scheduled today."
          >
            {dashboard?.today_schedule?.map((ev) => <DashboardItem key={ev.id} ev={ev} onClick={() => setDetailEvent(ev)} />)}
          </DashboardCard>
          ...
          <DashboardCard
            ...
            loading={dashLoading}
            empty={!dashboard?.upcoming_deadlines?.length}
            emptyText="No upcoming deadlines."
          >
            {dashboard?.upcoming_deadlines?.map((ev) => <DashboardItem key={ev.id} ev={ev} onClick={() => setDetailEvent(ev)} />)}
          </DashboardCard>
          ...
          <DashboardCard
            ...
            loading={dashLoading}
            empty={!dashboard?.upcoming_meetings?.length}
            emptyText="No upcoming meetings."
          >
            {dashboard?.upcoming_meetings?.map((ev) => <DashboardItem key={ev.id} ev={ev} onClick={() => setDetailEvent(ev)} />)}
          </DashboardCard>
          ...
          <DashboardCard
            ...
            loading={dashLoading}
            empty={!dashboard?.upcoming_tasks?.length}
            emptyText="No upcoming tasks."
          >
            {dashboard?.upcoming_tasks?.map((ev) => <DashboardItem key={ev.id} ev={ev} onClick={() => setDetailEvent(ev)} />)}
          </DashboardCard>
          ...
          <DashboardCard
            ...
            title="Birthdays"
            loading={dashLoading}
            empty={!dashboard?.birthdays?.length}
          ...
          <DashboardCard
            ...
            loading={dashLoading}
            empty={!dashboard?.employees_on_leave?.length}
...
const DashboardCard = ({ icon: Icon, title, loading, empty, emptyText, children }) => (
  <div className={styles.dashboardCard}>
    ...
    {loading ? (
      ...
      <div className={styles.dashboardList}>{children}</div>
    )}
  </div>
);

const DashboardItem = ({ ev, onClick }) => {
  const meta = EVENT_META[ev.event_type] || EVENT_META.Personal;
  const color = ev.color || meta.color;
  return (
    <button className={styles.dashboardItem} onClick={onClick}>
      <span className={styles.dashboardDot} style={{ background: color }} />
      <span className={styles.dashboardTitle}>{ev.title}</span>
